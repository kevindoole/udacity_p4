...
# - - - Registration - - - - - - - - - - - - - - - - - - - -
        ...
        for field in csf.all_fields():
            if hasattr(session, field.name):
                ...
                if field.name == 'date' or field.name == 'startTime':
                    setattr(csf, field.name, str(getattr(session, field.name)))
                else:
                    setattr(csf, field.name, getattr(session, field.name))
            elif field.name == "websafeKey":
                setattr(csf, field.name, session.key.urlsafe())
    ...
    @endpoints.method(ConferenceSessionForm, ConferenceSessionForm, path='session',
    ...
    def getConferenceSessions(self, request):
        ...
        # session_keys = [ndb.Key(urlsafe=wssk) for wssk in conference.sessionKeys]
        # sessions = ndb.get_multi(session_keys)

        sessions = ConferenceSession.query(
            ConferenceSession.websafeConferenceKey==request.websafeConferenceKey).fetch()

        return ConferenceSessionForms(items=[self._copyConferenceSessionToForm(session)\
